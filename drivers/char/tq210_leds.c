...
/* 用来指定LED所用的GPIO引脚 */
#if defined(CONFIG_TQ210_CODE_B)
static unsigned long gpio_table [] =
{
	S5PV210_GPC0(3),
	S5PV210_GPC0(4),
};
#elif defined(CONFIG_TQ210_CODE_C)
static unsigned long gpio_table [] =
{
	S5PV210_GPC1(3),
	S5PV210_GPC1(4),
...
static int tq210_gpio_open(struct inode *inode, struct file *file)
{
	...
	{
#if defined(CONFIG_TQ210_CODE_B)
		printk(KERN_ERR "failed to request GPC0_3 for LED1 pin\n");
#elif defined(CONFIG_TQ210_CODE_C)
		printk(KERN_ERR "failed to request GPC1_3 for LED1 pin\n");
#endif
        return err;
	}
#if defined(CONFIG_TQ210_CODE_B)
	...
	{
#if defined(CONFIG_TQ210_CODE_B)
		printk(KERN_ERR "failed to request GPC0_4 for LED2 pin\n");
#elif defined(CONFIG_TQ210_CODE_C)
		printk(KERN_ERR "failed to request GPC1_4 for LED2 PWDN pin\n");
#endif
        return err;
	}
...
static int __init dev_init(void)
{
	...
	if(err)
	{
		...
		printk(KERN_ERR "failed to request GPC0_3 for LED1 pin\n");
#elif defined(CONFIG_TQ210_CODE_C)
		printk(KERN_ERR "failed to request GPC1_3 for LED1 PWDN pin\n");
#endif
		return err;
	}
#if defined(CONFIG_TQ210_CODE_B)
	...
	if(err)
	{
		...
		printk(KERN_ERR "failed to request GPC0_4 for LED2 pin\n");
#elif defined(CONFIG_TQ210_CODE_C)
		printk(KERN_ERR "failed to request GPC1_4 for LED2 pin\n");
#endif
		return err;
	}
	for (i = 0; i < sizeof(gpio_table)/sizeof(unsigned long); i++)
