...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    input_mt_init_slots(ts->input_dev, 1);     // in case of "out of memory"
#else
...
#endif 
#if 0
    log_msg(LOG_DEBUG, "set input para \n");
	input_dev->name		= "Capacitance_ts";//FT5X0X_NAME;		//dev_name(&client->dev)
	input_dev->phys = "I2C";
    ...
    input_set_abs_params(input_dev, ABS_X, 0, ts->abs_x_max, 0, 0);
    input_set_abs_params(input_dev, ABS_Y, 0, ts->abs_y_max, 0, 0);
