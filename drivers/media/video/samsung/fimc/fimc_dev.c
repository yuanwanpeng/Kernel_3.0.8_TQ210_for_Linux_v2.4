...
#ifdef MDF_ZQL
#include"log_msg.h"
int phy_write32(unsigned long phyaddr,unsigned int dt)
{
/*	gpio_request(S5PV210_MP01(1), "fimc_mclk");
	s3c_gpio_cfgpin(S5PV210_MP01(1), S3C_GPIO_SFN(2));
	gpio_free(S5PV210_MP01(1));*/
	unsigned int *pvirt;
	struct resource *res;

	res = request_mem_region(phyaddr,4,"tmp");
	if(res == NULL)
	{
		log_msg(LOG_ERROR,"can't request mem(%08x)\n",phyaddr);
		return -1;
	}
	pvirt = ioremap(phyaddr,4);
	if(pvirt == NULL)
	{
		log_msg(LOG_ERROR,"can't ioremap mem(%08x)\n",phyaddr);
		return -1;
	}
	iowrite32(dt,pvirt);
	iounmap(pvirt);
	release_mem_region(phyaddr,4);
	return 0;
}

int phy_read32(unsigned long phyaddr,unsigned int *dt)
{
	unsigned int *pvirt;
	struct resource *res;
...
#ifdef MDF_ZQL
	phy_write32(0xe0200100,0x00012222);
#endif /* MDF_ZQL */
	return 0;
