				...
				unsigned char length)
{
	...
	sccb_write16((client->addr)<<1, (unint16)(i2c_data[0]<<8)|(i2c_data[1]),i2c_data[2]);
	return 0;
}
static int ov3640_read(struct v4l2_subdev *sd, unsigned short addr)
{
	unsigned char dt;
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	dt = sccb_read16((client->addr)<<1, addr);
	printk("subaddr(%2x),addr(%2x)=%02x\n",(client->addr)<<1,addr,dt);
	return dt;
}
#else
static int ov3640_i2c_write(struct v4l2_subdev *sd, unsigned char i2c_data[],
...
int checkIfOV3640(struct v4l2_subdev *sd)
{
	...
	for(i=0;i<5;++i)
	{
		pid = (ov3640_read(sd, 0x300a)<<8)|(ov3640_read(sd, 0x300b));	
		if(pid == 0x364c)
		{
//			printk("OV3640 address 0x%02X, product ID 0x%04X, expect 0x%04X\n", sd, pid, 0x364c);
			return 0;				
		}	
		mdelay(20);
	}
...
static int ov3640_s_config(struct v4l2_subdev *sd, int irq, void *platform_data)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ov3640_state *state = to_state(sd);
	struct tqcam_platform_data *pdata;
