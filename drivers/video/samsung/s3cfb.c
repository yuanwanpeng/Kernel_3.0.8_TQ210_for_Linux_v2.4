...
void s3cfb_early_suspend_ghost(struct s3cfb_global *fbdev)
{
	struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);	
...
void s3cfb_late_resume_ghost(struct s3cfb_global *fbdev)
{
	struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
	struct platform_device *pdev = to_platform_device(fbdev->dev);
	struct fb_info *fb;
...
//---------------------------------------------
s3cfb_early_suspend_ghost(fbdev);
msleep(500);
s3cfb_late_resume_ghost(fbdev);
	return 0;
